...
const LiveGame: React.FC = () => {
  ...
  useEffect(() => {
    if (gameData?.game) {
      ...
      const initHomeStats: PlayerStats[] = gameData.game.home_team.players?.map((player: Player) => ({
        playerId: player.id,
        points: 0,
        rebounds: 0,
        assists: 0,
      ...
      const initAwayStats: PlayerStats[] = gameData.game.away_team.players?.map((player: Player) => ({
        playerId: player.id,
        points: 0,
        rebounds: 0,
        assists: 0,
    ...
    <div className="min-h-screen bg-gray-900 p-6">
      ...
      <div className="bg-gray-800 rounded-lg p-6 mb-8 border border-gray-700">
        <div className="grid grid-cols-3 gap-8 items-center">
          {/* Away Team */}
          <div className="text-center">
            <h2 className="text-xl font-bold text-white mb-2">{game.away_team.name}</h2>
            <div className="text-5xl font-bold text-orange-500">{gameState.awayScore}</div>
          ...
          <div className="text-center">
            <div className="bg-gray-900 rounded-lg p-4 mb-4">
              <div className="text-sm text-gray-400 mb-1">PERIOD {gameState.period}</div>
          ...
          {/* Home Team */}
          <div className="text-center">
            <h2 className="text-xl font-bold text-white mb-2">{game.home_team.name}</h2>
            <div className="text-5xl font-bold text-orange-500">{gameState.homeScore}</div>
            <div className="mt-4 space-x-2">
              <button
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-bold text-lg">{gameState.awayScore}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">{game.home_team.name}:</span>
                  <span className="text-white font-bold text-lg">{gameState.homeScore}</span>
                </div>
      ...
      {activeTab === 'stats' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Away Team Stats */}
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <h3 className="text-lg font-semibold text-white mb-4">{game.away_team.name} - Player Stats</h3>
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b border-gray-600">
                    ...
                    <th className="text-center py-2 px-2 text-xs font-medium text-gray-300 uppercase">FOULS</th>
                  ...
                  {awayPlayers.map((playerStat) => {
                    ...
                    return (
                      <tr key={playerStat.playerId}>
                        ...
                        <td className="py-2 px-2 text-center text-sm text-white">{playerStat.assists}</td>
                        <td className="py-2 px-2 text-center text-sm text-white">{playerStat.fouls}</td>
                        <td className="py-2 px-2 text-center">
                          <div className="flex justify-center space-x-1">
                            <button
                              onClick={() => updatePlayerStat(playerStat.playerId, 'away', 'points', 2)}
                              className="px-1 py-1 text-xs bg-green-600 text-white rounded hover:bg-green-700"
                            >
                              +2
                            </button>
                            <button
                              onClick={() => updatePlayerStat(playerStat.playerId, 'away', 'rebounds', 1)}
                              className="px-1 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700"
                            >
                              +R
                            </button>
                            <button
                              onClick={() => updatePlayerStat(playerStat.playerId, 'away', 'assists', 1)}
          ...
          {/* Home Team Stats */}
          <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <h3 className="text-lg font-semibold text-white mb-4">{game.home_team.name} - Player Stats</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead>
                  <tr className="border-b border-gray-600">
                    ...
                    return (
                      <tr key={playerStat.playerId}>
                        ...
                        <td className="py-2 px-2 text-center">
                          <div className="flex justify-center space-x-1">
                            <button
                              onClick={() => updatePlayerStat(playerStat.playerId, 'home', 'points', 2)}
                              className="px-1 py-1 text-xs bg-green-600 text-white rounded hover:bg-green-700"
                            >
                              +2
                            </button>
                            <button
                              onClick={() => updatePlayerStat(playerStat.playerId, 'home', 'rebounds', 1)}
                              className="px-1 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700"
                            >
                              +R
                            </button>
                            <button
                              onClick={() => updatePlayerStat(playerStat.playerId, 'home', 'assists', 1)}
                              className="px-1 py-1 text-xs bg-purple-600 text-white rounded hover:bg-purple-700"
                            >
                              +A
                            </button>
      ...
      {activeTab === 'substitutions' && (
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h3 className="text-lg font-semibold text-white mb-4">Player Substitutions</h3>
