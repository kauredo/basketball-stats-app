...
export const getPageStyles = (props: PageProps = {}) => {
  const { variant = "default", padding = "md", maxWidth = "full" } = props;

  const baseClasses = ["min-h-screen", "bg-gradient-to-br from-dark-950 to-dark-900"].join(" ");
...
export const getContentStyles = (animated: boolean = true) => {
  return ["flex-1", animated ? "animate-fade-in" : ""].filter(Boolean).join(" ");
};
...
export const getBasketballIconStyles = (
  size: "sm" | "md" | "lg" | "xl" = "md",
  ...
  return [sizeMap[size], animate ? "animate-bounce-ball" : "", "mb-4"].filter(Boolean).join(" ");
};
