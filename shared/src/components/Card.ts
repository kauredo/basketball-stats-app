...
export const getCardStyles = (props: CardProps = {}) => {
  const { variant = "default", padding = "md", hover = false, interactive = false } = props;
  ...
  const variantClasses = {
    ...
    elevated: ["bg-white dark:bg-dark-800", "shadow-md hover:shadow-lg"].join(" "),

    outlined: ["bg-transparent", "border border-gray-300 dark:border-dark-600"].join(" "),
...
export const getStatValueStyles = (
  color: "primary" | "success" | "warning" | "danger" | "neutral" = "primary"
) => {
  const colorMap = {
