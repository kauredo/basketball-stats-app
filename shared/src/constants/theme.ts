...
export const STAT_BUTTON_CONFIG = {
  scoring: [
    { key: 'shot2', label: '2PT', shortLabel: '+2', color: COLORS.statButtons.scoring, requiresMade: true },
    { key: 'shot3', label: '3PT', shortLabel: '+3', color: COLORS.statButtons.scoring, requiresMade: true },
    { key: 'freethrow', label: 'FT', shortLabel: '+1', color: COLORS.statButtons.scoring, requiresMade: true },
  ],
  playmaking: [
    { key: 'assists', label: 'AST', shortLabel: '+A', color: COLORS.statButtons.playmaking, requiresMade: false },
  ],
  defense: [
    { key: 'steals', label: 'STL', shortLabel: '+S', color: COLORS.statButtons.defense, requiresMade: false },
    { key: 'blocks', label: 'BLK', shortLabel: '+B', color: COLORS.statButtons.defense, requiresMade: false },
  ],
  rebounding: [
    { key: 'rebounds', label: 'REB', shortLabel: '+R', color: COLORS.statButtons.rebounding, requiresMade: false },
  ],
  negative: [
    { key: 'turnovers', label: 'TO', shortLabel: '+T', color: COLORS.statButtons.negative, requiresMade: false },
    { key: 'fouls', label: 'FOUL', shortLabel: '+F', color: COLORS.statButtons.negative, requiresMade: false },
  ],
} as const;
...
export function getShotZone(x: number, y: number): keyof typeof SHOT_ZONES | 'unknown' {
  ...
  const isThreePointer = distanceFromBasket > COURT_DIMENSIONS.threePointDistance ||
    (Math.abs(x) > 22 && y < 14); // Corner 3s
