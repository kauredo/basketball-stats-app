...
export const FancyButton = ({ className, glow = false, ...props }: FancyButtonProps) => (
  <Button
    className={cn(
...
export const FormField = ({ label, error, ...inputProps }: FormFieldProps) => (
  <div className="space-y-2">
    <Label>{label}</Label>
    <Input className={cn(error && "border-destructive")} aria-invalid={!!error} {...inputProps} />
...
:root {
  ...
  /* Surface effects */
  --surface-shine: linear-gradient(135deg, oklch(100% 0 0 / 0.1) 0%, transparent 50%);
